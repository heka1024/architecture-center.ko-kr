...
이 문서에서는 Azure에서 Windows VM(가상 머신)을 실행하는 방법에 대한 검증된 사례를 보여줍니다. 여기에는 VM 프로비전과 함께 네트워킹 및 저장소 구성 요소에 관한 권장 사항이 포함됩니다. [**이 솔루션을 배포합니다**](#deploy-the-solution).
...
- **관리되는 디스크**. [Azure Managed Disks][managed-disks]는 저장소를 처리하여 디스크 관리를 단순화합니다. OS 디스크는 [Azure Storage][azure-storage]에 저장된 VHD이므로 호스트 컴퓨터가 중단되어도 계속 유지됩니다. 또한 애플리케이션 데이터에 사용되는 영구 VHD인 [데이터 디스크][data-disk]를 하나 이상 만드는 것이 좋습니다.
...
- **가상 네트워크(VNet)**. 모든 Azure VM은 VNet에 배포되어 여러 서브넷으로 분할될 수 있습니다.
...
하나 이상의 데이터 디스크를 추가합니다. VHD를 만들 때 형식은 지정되지 않습니다. 디스크를 포맷하려면 VM에 로그인합니다. 가능한 경우 OS 디스크가 아닌 데이터 디스크에 애플리케이션을 설치합니다. 일부 기존 응용 프로그램은 C: 드라이브에 구성 요소를 설치해야 할 수 있습니다. 이 경우 PowerShell을 사용하여 [OS 디스크 크기를 조정][resize-os-disk]할 수 있습니다.
