...
성공적인 명명 규칙이 되기 위한 핵심은 애플리케이션과 조직 전체에서 규칙을 정하여 따르는 것입니다.
...
- 제품 라인은 부서 내에서 수행되는 제품 또는 기능에 대한 특정 이름입니다. 일반적으로 내부 전용 서비스 및 애플리케이션에는 선택 사항입니다. 쉬운 분리 및 ID(예: 청구 레코드의 명확한 분리)가 필요한 공용 서비스에 사용하는 것이 좋습니다.
- 환경은 개발, QA 또는 프로덕션 등의 애플리케이션 또는 서비스의 배포 수명 주기를 설명하는 이름입니다.
...
대규모 기업에 대해 구독을 구성하는 방법에 대한 자세한 내용은 [Azure 엔터프라이즈 스캐폴드 - 규범적 구독 거버넌스](/azure/architecture/cloud-adoption/appendix/azure-scaffold)를 참조하세요.
...
|API Management |전역 |1-50 |대/소문자 구분하지 않음 |영숫자 및 하이픈 |`<apim-service-name>` |`contoso` |
...
|Function App | 전역 |1-60 |대/소문자 구분하지 않음 |영숫자 및 하이픈 |`<name>-func` |`calcprofit-func` |
...
- **서비스 컨텍스트 식별**. 일반 작업 및 그룹화에 대한 리소스 그룹의 리소스 그룹을 식별합니다.
- **Access Control 및 보안 컨텍스트**. 포트폴리오, 시스템, 서비스, 앱, 인스턴스 등을 기준으로 관리 역할을 식별합니다.

> [!TIP]
> 초기에 자주 태그 지정. 사후에 개/보수하는 것보다 기준 태그 지정 체계를 준비하고 시간에 따라 조정하는 것이 좋습니다.
