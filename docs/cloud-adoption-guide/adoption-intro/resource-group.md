...
- 리소스 그룹의 모든 리소스는 동일한 수명 주기를 가져야 합니다. 즉, 그룹과 동일한 수명 주기를 사용하여 리소스를 배포, 업데이트 및 삭제하는 방식을 따라야 합니다. 예를 들어 단일 웹 애플리케이션의 계산 리소스는 일반적으로 하나의 단위로 배포됩니다. 그러나 다른 웹 애플리케이션과 공유되는 데이터베이스는 다른 수명 주기에 따라 관리되기 쉬우며, 자체 리소스 그룹에 속하는 것이 좋습니다.
