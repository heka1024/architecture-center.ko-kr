...
- 실시간 분석을 수행하려고 하나요? 그렇다면 실시간 분석을 지원하는 옵션으로 범위를 줄입니다.
    ...
    이 컨텍스트의 *실시간 분석*은 운영 및 분석 워크로드를 둘 다 실행하는 ERP(전사적 자원 관리) 애플리케이션 같은 단일 데이터 원본에 적용됩니다. 여러 원본의 데이터를 통합해야 하거나 큐브와 같은 사전 집계된 데이터를 사용하여 분석 성능을 높여야 할 경우 여전히 별도의 데이터 웨어하우스가 필요할 수 있습니다.
...
| 다차원 큐브 지원 여부 | 아니요 | 예 | 아니요 | 아니요 |
| 테이블 형식 의미 체계 모델 지원 여부 | 예 | 예 | 아니요 | 아니요 |
...
|                                                  | Azure Analysis Services | SQL Server Analysis Services | SQL Server(columnstore 인덱스 포함) | Azure SQL Database(columnstore 인덱스 포함) |
|--------------------------------------------------|-------------------------|------------------------------|-------------------------------------|---------------------------------------------|
| 고가용성을 위한 중복 지역 서버 |           예           |              no              |                 예                 |                     예                     |
|             쿼리 확장 지원 여부             |           예           |              no              |                 예                 |                     아니요                      |
|          동적 확장성(강화)          |           예           |              no              |                 예                 |                     아니요                      |
