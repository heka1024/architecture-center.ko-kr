...
일반적으로 문서에는 엔터티의 전체 데이터가 포함됩니다. 엔터티를 구성하는 항목은 애플리케이션에 따라 다릅니다. 예를 들어, 엔터티에는 고객의 세부 정보, 주문 또는 이 두 정보의 조합을 포함할 수 있습니다. RDBMS(관계형 데이터베이스 관리 시스템)의 여러 관계형 테이블에 분산된 정보가 단일 문서에 포함될 수도 있습니다. 문서 저장소의 모든 문서가 동일한 구조를 가질 필요는 없습니다. 이 자유 형식 접근 방식은 많은 유연성을 제공합니다. 예를 들어, 비즈니스 요구 사항의 변경에 대한 응답으로, 응용 프로그램은 문서에 다른 데이터를 저장할 수 있습니다.  
