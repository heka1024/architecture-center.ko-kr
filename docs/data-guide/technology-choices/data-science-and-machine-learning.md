...
- **Visual Studio Code**: 기계 학습 및 AI 개발을 위해 일반적으로 사용되는 프레임워크일 뿐만 아니라 Python을 지원하는 경량, 플랫폼 간 코딩 환경입니다.
...
Azure Machine Learning은 Machine Learning 실험 및 모델을 관리하기 위한 클라우드 기반 서비스입니다. 여기에는 데이터 준비 및 모델링 학습 스크립트를 추적하는 실험 서비스가 포함되며, 반복 간에 모델 성능을 비교할 수 있도록 모든 실행 기록이 유지 관리됩니다. 데이터 과학자는 Jupyter Notebook 또는 Visual Studio Code와 같이 선택한 도구에서 스크립트를 만든 다음, Azure에서 다른 다양한 [컴퓨팅 리소스](/azure/machine-learning/service/how-to-set-up-training-targets)에 배포할 수 있습니다.

Docker 컨테이너, Azure HDinsight의 Spark, Microsoft Machine Learning Server 또는 SQL Server에 웹 서비스로 모델을 배포할 수 있습니다. 그런 다음, Azure Machine Learning 모델 관리 서비스를 사용하여 클라우드, 에지 디바이스 또는 엔터프라이즈 전체에서 모델 배포를 추적하고 관리할 수 있습니다.
...
- HDinsight의 Spark로 직접 모델을 배포하고, Azure Machine Learning 모델 관리 서비스를 사용하여 관리할 수 있습니다.
