...
* 등록은 애플리케이션 데이터베이스에 사용자의 테넌트를 추가합니다.
* 테넌트를 등록한 후 애플리케이션은 온보딩 페이지를 보여 줍니다.

이 섹션에서 등록 프로세스의 구현을 안내합니다.
"등록" 및 "로그인"은 애플리케이션 개념이라는 점을 이해해야 합니다. 인증 흐름 중 Azure AD는 사용자가 등록 프로세스에 있는지 여부를 본질적으로 알지 못합니다. 컨텍스트 추적은 애플리케이션에 달려 있습니다.
