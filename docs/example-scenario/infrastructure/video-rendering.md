...
Batch는 Windows Server 또는 Linux 계산 노드 중 어느 것을 선택하든지 간에 일관된 관리 환경과 작업 예약을 제공합니다. Batch를 사용하면 AutoDesk Maya 및 Blender를 포함하여 기존 Windows 또는 Linux 애플리케이션을 통해 Azure에서 대규모 렌더 작업을 실행할 수 있습니다.
...
이 시나리오에서는 Azure Batch를 사용하는 워크플로를 보여줍니다. 데이터는 다음과 같이 흐릅니다.
...
### <a name="pricing-for-low-priority-vms"></a>우선 순위가 낮은 VM의 가격 책정
...
> [!NOTE] 
> 우선 순위가 낮은 VM은 특정 응용 프로그램 및 워크로드에만 적합합니다.
