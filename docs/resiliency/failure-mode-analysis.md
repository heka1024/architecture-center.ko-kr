...
<span data-ttu-id="4baf9-183">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-183">**Recovery**</span></span>
...
## <a name="cassandra"></a><span data-ttu-id="4baf9-202">Cassandra</span><span class="sxs-lookup"><span data-stu-id="4baf9-202">Cassandra</span></span>
...
<span data-ttu-id="4baf9-243">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-243">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-244">클라이언트 쪽에서 모든 오류를 기록합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-244">Log all errors on the client side.</span></span>
...
## <a name="elasticsearch"></a><span data-ttu-id="4baf9-264">Elasticsearch</span><span class="sxs-lookup"><span data-stu-id="4baf9-264">Elasticsearch</span></span>
...
<span data-ttu-id="4baf9-283">자세한 내용은 [Azure에서 Elasticsearch 실행][elasticsearch-azure]을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-283">For more information, see [Running Elasticsearch on Azure][elasticsearch-azure].</span></span>
...
<span data-ttu-id="4baf9-300">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-300">**Recovery**</span></span>
...
### <a name="writing-to-the-cache-fails"></a><span data-ttu-id="4baf9-318">캐시에 쓰는 작업이 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-318">Writing to the cache fails.</span></span>
...
<span data-ttu-id="4baf9-337">복제본은 다른 연결 문자열을 사용하므로 응용 프로그램에서 연결 문자열을 업데이트해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-337">The replica uses a different connection string, so you will need to update the connection string in your application.</span></span>
...
### <a name="writing-a-message-to-a-service-bus-queue-fails"></a><span data-ttu-id="4baf9-376">Service Bus 큐에 메시지를 쓰는 작업이 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-376">Writing a message to a Service Bus queue fails.</span></span>
...
<span data-ttu-id="4baf9-419">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-419">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-420">중복 메시지를 기록합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-420">Log duplicated messages.</span></span>
...
<span data-ttu-id="4baf9-441">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-441">**Recovery**</span></span>
...
<span data-ttu-id="4baf9-464">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-464">**Recovery**</span></span>
...
3. <span data-ttu-id="4baf9-468">N회의 다시 시도가 실패하면 정상적인 대체(fallback)를 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-468">If N retry attempts fail, perform a graceful fallback.</span></span> <span data-ttu-id="4baf9-469">예: </span><span class="sxs-lookup"><span data-stu-id="4baf9-469">For example:</span></span>
   ...
   * <span data-ttu-id="4baf9-470">로컬 캐시에 데이터를 저장하고, 나중에 서비스를 사용할 수 있게 되면 저장소에 쓰기를 전달합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-470">Store the data in a local cache, and forward the writes to storage later, when the service becomes available.</span></span>
   * <span data-ttu-id="4baf9-471">쓰기 작업이 트랜잭션 범위에 있는 경우 트랜잭션을 보정합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-471">If the write action was in a transactional scope, compensate the transaction.</span></span>

<span data-ttu-id="4baf9-472">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-472">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-473">[저장소 메트릭][storage-metrics]을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-473">Use [storage metrics][storage-metrics].</span></span>

### <a name="reading-data-from-azure-storage-fails"></a><span data-ttu-id="4baf9-474">Azure Storage에서 데이터를 읽는 작업이 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-474">Reading data from Azure Storage fails.</span></span>
<span data-ttu-id="4baf9-475">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-475">**Detection**.</span></span> <span data-ttu-id="4baf9-476">읽기 작업 중에 클라이언트에서 오류를 받습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-476">The client receives errors when reading.</span></span>

<span data-ttu-id="4baf9-477">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-477">**Recovery**</span></span>

1. <span data-ttu-id="4baf9-478">작업을 다시 시도하여 일시적인 장애로부터 복구합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-478">Retry the operation, to recover from transient failures.</span></span> <span data-ttu-id="4baf9-479">클라이언트 SDK의 [다시 시도 정책][Storage.RetryPolicies]에서 이 작업을 자동으로 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-479">The [retry policy][Storage.RetryPolicies] in the client SDK handles this automatically.</span></span>
2. <span data-ttu-id="4baf9-480">RA-GRS 저장소의 경우 기본 엔드포인트에서 읽기 작업이 실패하면 보조 엔드포인트에서 읽기 작업을 시도합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-480">For RA-GRS storage, if reading from the primary endpoint fails, try reading from the secondary endpoint.</span></span> <span data-ttu-id="4baf9-481">클라이언트 SDK에서 이 작업을 자동으로 처리할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-481">The client SDK can handle this automatically.</span></span> <span data-ttu-id="4baf9-482">[Azure Storage 복제][storage-replication]를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-482">See [Azure Storage replication][storage-replication].</span></span>
3. <span data-ttu-id="4baf9-483">*N*회의 다시 시도가 실패하면 대체(fallback)를 수행하여 성능을 정상적으로 낮춥니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-483">If *N* retry attempts fail, take a fallback action to degrade gracefully.</span></span> <span data-ttu-id="4baf9-484">예를 들어 제품 이미지를 저장소에서 검색할 수 없는 경우 일반적인 자리 표시자 이미지가 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-484">For example, if a product image can't be retrieved from storage, show a generic placeholder image.</span></span>

<span data-ttu-id="4baf9-485">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-485">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-486">[저장소 메트릭][storage-metrics]을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-486">Use [storage metrics][storage-metrics].</span></span>

## <a name="virtual-machine"></a><span data-ttu-id="4baf9-487">Virtual Machine</span><span class="sxs-lookup"><span data-stu-id="4baf9-487">Virtual Machine</span></span>
### <a name="connection-to-a-backend-vm-fails"></a><span data-ttu-id="4baf9-488">백 엔드 VM에 대한 연결이 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-488">Connection to a backend VM fails.</span></span>
<span data-ttu-id="4baf9-489">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-489">**Detection**.</span></span> <span data-ttu-id="4baf9-490">네트워크 연결 오류입니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-490">Network connection errors.</span></span>

<span data-ttu-id="4baf9-491">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-491">**Recovery**</span></span>

* <span data-ttu-id="4baf9-492">부하 분산 장치 뒤에 있는 가용성 집합에 둘 이상의 백 엔드 VM을 배포합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-492">Deploy at least two backend VMs in an availability set, behind a load balancer.</span></span>
* <span data-ttu-id="4baf9-493">일시적인 연결 오류이면 TCP에서 메시지 전송을 성공적으로 다시 시도하는 경우가 있습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-493">If the connection error is transient, sometimes TCP will successfully retry sending the message.</span></span>
* <span data-ttu-id="4baf9-494">응용 프로그램에서 다시 시도 정책을 구현합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-494">Implement a retry policy in the application.</span></span>
* <span data-ttu-id="4baf9-495">영구 오류 또는 일시적이지 않은 오류의 경우 [회로 차단기][circuit-breaker] 패턴을 구현합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-495">For persistent or non-transient errors, implement the [Circuit Breaker][circuit-breaker] pattern.</span></span>
* <span data-ttu-id="4baf9-496">호출하는 VM에서 네트워크 송신 제한을 초과하면 아웃바운드 큐가 채워집니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-496">If the calling VM exceeds its network egress limit, the outbound queue will fill up.</span></span> <span data-ttu-id="4baf9-497">아웃바운드 큐가 일관되게 가득 차면 크기를 확장하는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-497">If the outbound queue is consistently full, consider scaling out.</span></span>

<span data-ttu-id="4baf9-498">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-498">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-499">서비스 경계에서 이벤트를 로깅합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-499">Log events at service boundaries.</span></span>

### <a name="vm-instance-becomes-unavailable-or-unhealthy"></a><span data-ttu-id="4baf9-500">VM 인스턴스가 사용할 수 없거나 비정상적인 상태가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-500">VM instance becomes unavailable or unhealthy.</span></span>
<span data-ttu-id="4baf9-501">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-501">**Detection**.</span></span> <span data-ttu-id="4baf9-502">VM 인스턴스가 정상인지 여부를 나타내는 Load Balancer [상태 프로브][lb-probe]를 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-502">Configure a Load Balancer [health probe][lb-probe] that signals whether the VM instance is healthy.</span></span> <span data-ttu-id="4baf9-503">프로브를 통해 중요한 기능이 올바르게 응답하는지 여부를 확인해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-503">The probe should check whether critical functions are responding correctly.</span></span>

<span data-ttu-id="4baf9-504">**복구**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-504">**Recovery**.</span></span> <span data-ttu-id="4baf9-505">각 응용 프로그램 계층에 대해 여러 VM 인스턴스를 동일한 가용성 집합에 배치하고, VM 앞에 Load Balancer를 배치합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-505">For each application tier, put multiple VM instances into the same availability set, and place a load balancer in front of the VMs.</span></span> <span data-ttu-id="4baf9-506">상태 프로브가 실패하면 Load Balancer에서 비정상 인스턴스에 대한 새 연결 전송을 중지합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-506">If the health probe fails, the Load Balancer stops sending new connections to the unhealthy instance.</span></span>

<span data-ttu-id="4baf9-507">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-507">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-508">Load Balancer [로그 분석][lb-monitor]을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-508">- Use Load Balancer [log analytics][lb-monitor].</span></span>

* <span data-ttu-id="4baf9-509">모든 상태 모니터링 엔드포인트를 모니터링하도록 모니터링 시스템을 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-509">Configure your monitoring system to monitor all of the health monitoring endpoints.</span></span>

### <a name="operator-accidentally-shuts-down-a-vm"></a><span data-ttu-id="4baf9-510">운영자가 실수로 VM을 종료합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-510">Operator accidentally shuts down a VM.</span></span>
<span data-ttu-id="4baf9-511">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-511">**Detection**.</span></span> <span data-ttu-id="4baf9-512">해당 없음</span><span class="sxs-lookup"><span data-stu-id="4baf9-512">N/A</span></span>

<span data-ttu-id="4baf9-513">**복구**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-513">**Recovery**.</span></span> <span data-ttu-id="4baf9-514">`ReadOnly` 수준의 리소스 잠금을 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-514">Set a resource lock with `ReadOnly` level.</span></span> <span data-ttu-id="4baf9-515">[Azure Resource Manager를 사용하여 리소스 잠그기][rm-locks]를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-515">See [Lock resources with Azure Resource Manager][rm-locks].</span></span>

<span data-ttu-id="4baf9-516">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-516">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-517">[Azure 활동 로그][azure-activity-logs]를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-517">Use [Azure Activity Logs][azure-activity-logs].</span></span>

## <a name="webjobs"></a><span data-ttu-id="4baf9-518">웹 작업</span><span class="sxs-lookup"><span data-stu-id="4baf9-518">WebJobs</span></span>
### <a name="continuous-job-stops-running-when-the-scm-host-is-idle"></a><span data-ttu-id="4baf9-519">SCM 호스트가 유휴 상태일 때 연속 작업의 실행이 중지됩니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-519">Continuous job stops running when the SCM host is idle.</span></span>
<span data-ttu-id="4baf9-520">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-520">**Detection**.</span></span> <span data-ttu-id="4baf9-521">취소 토큰을 WebJob 함수에 전달합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-521">Pass a cancellation token to the WebJob function.</span></span> <span data-ttu-id="4baf9-522">자세한 내용은 [정상 종료][web-jobs-shutdown]를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-522">For more information, see [Graceful shutdown][web-jobs-shutdown].</span></span>

<span data-ttu-id="4baf9-523">**복구**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-523">**Recovery**.</span></span> <span data-ttu-id="4baf9-524">웹앱에서 `Always On` 설정을 사용하도록 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-524">Enable the `Always On` setting in the web app.</span></span> <span data-ttu-id="4baf9-525">자세한 내용은 [WebJobs로 백그라운드 작업 실행][web-jobs]을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-525">For more information, see [Run Background tasks with WebJobs][web-jobs].</span></span>

## <a name="application-design"></a><span data-ttu-id="4baf9-526">애플리케이션 설계</span><span class="sxs-lookup"><span data-stu-id="4baf9-526">Application design</span></span>
### <a name="application-cant-handle-a-spike-in-incoming-requests"></a><span data-ttu-id="4baf9-527">응용 프로그램에서 들어오는 요청의 스파이크를 처리할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-527">Application can't handle a spike in incoming requests.</span></span>
<span data-ttu-id="4baf9-528">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-528">**Detection**.</span></span> <span data-ttu-id="4baf9-529">응용 프로그램에 따라 다릅니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-529">Depends on the application.</span></span> <span data-ttu-id="4baf9-530">일반적인 증상은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-530">Typical symptoms:</span></span>

* <span data-ttu-id="4baf9-531">웹 사이트에서 HTTP 5xx 오류 코드를 반환하기 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-531">The website starts returning HTTP 5xx error codes.</span></span>
* <span data-ttu-id="4baf9-532">데이터베이스 또는 저장소와 같은 종속 서비스에서 요청을 제한하기 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-532">Dependent services, such as database or storage, start to throttle requests.</span></span> <span data-ttu-id="4baf9-533">서비스에 따라 HTTP 429(너무 많은 요청)와 같은 HTTP 오류를 찾습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-533">Look for HTTP errors such as HTTP 429 (Too Many Requests), depending on the service.</span></span>
* <span data-ttu-id="4baf9-534">HTTP 큐 길이가 늘어납니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-534">HTTP queue length grows.</span></span>

<span data-ttu-id="4baf9-535">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-535">**Recovery**</span></span>

* <span data-ttu-id="4baf9-536">규모를 확장하여 증가된 부하를 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-536">Scale out to handle increased load.</span></span>
* <span data-ttu-id="4baf9-537">연속 장애로 인해 전체 응용 프로그램이 중단되지 않도록 장애를 완화합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-537">Mitigate failures to avoid having cascading failures disrupt the entire application.</span></span> <span data-ttu-id="4baf9-538">완화 전략은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-538">Mitigation strategies include:</span></span>

  * <span data-ttu-id="4baf9-539">[패턴 제한][throttling-pattern]을 구현하여 백 엔드 시스템의 과부하를 방지합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-539">Implement the [Throttling Pattern][throttling-pattern] to avoid overwhelming backend systems.</span></span>
  * <span data-ttu-id="4baf9-540">[큐 기반 부하 평준화][queue-based-load-leveling]를 사용하여 요청을 버퍼링하고 적절한 속도로 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-540">Use [queue-based load leveling][queue-based-load-leveling] to buffer requests and process them at an appropriate pace.</span></span>
  * <span data-ttu-id="4baf9-541">특정 클라이언트의 우선 순위를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-541">Prioritize certain clients.</span></span> <span data-ttu-id="4baf9-542">예를 들어 응용 프로그램에 체험 계층과 유료 계층이 있는 경우 체험 계층의 고객은 제한하고 유료 계층의 고객은 제한하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-542">For example, if the application has free and paid tiers, throttle customers on the free tier, but not paid customers.</span></span> <span data-ttu-id="4baf9-543">[우선 순위 큐 패턴][priority-queue-pattern]을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-543">See [Priority queue pattern][priority-queue-pattern].</span></span>

<span data-ttu-id="4baf9-544">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-544">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-545">[App Service 진단 로깅][app-service-logging]을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-545">Use [App Service diagnostic logging][app-service-logging].</span></span> <span data-ttu-id="4baf9-546">[Azure Log Analytics][azure-log-analytics], [Application Insights][app-insights] 또는 [New Relic][new-relic]과 같은 서비스를 사용하면 진단 로그를 이해하는 데 도움이 됩니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-546">Use a service such as [Azure Log Analytics][azure-log-analytics], [Application Insights][app-insights], or [New Relic][new-relic] to help understand the diagnostic logs.</span></span>

### <a name="one-of-the-operations-in-a-workflow-or-distributed-transaction-fails"></a><span data-ttu-id="4baf9-547">워크플로 또는 분산 트랜잭션의 작업 중 하나가 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-547">One of the operations in a workflow or distributed transaction fails.</span></span>
<span data-ttu-id="4baf9-548">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-548">**Detection**.</span></span> <span data-ttu-id="4baf9-549">*N*회의 다시 시도를 수행한 후에도 계속 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-549">After *N* retry attempts, it still fails.</span></span>

<span data-ttu-id="4baf9-550">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-550">**Recovery**</span></span>

* <span data-ttu-id="4baf9-551">완화 계획으로, [스케줄러 에이전트 감독자][scheduler-agent-supervisor] 패턴을 구현하여 전체 워크플로를 관리합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-551">As a mitigation plan, implement the [Scheduler Agent Supervisor][scheduler-agent-supervisor] pattern to manage the entire workflow.</span></span>
* <span data-ttu-id="4baf9-552">시간 제한 시 다시 시도를 수행하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-552">Don't retry on timeouts.</span></span> <span data-ttu-id="4baf9-553">이 오류에 대한 성공률은 낮습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-553">There is a low success rate for this error.</span></span>
* <span data-ttu-id="4baf9-554">나중에 다시 시도하기 위해 큐에 작업을 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-554">Queue work, in order to retry later.</span></span>

<span data-ttu-id="4baf9-555">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-555">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-556">보정 작업을 포함하여 모든 작업(성공 및 실패)을 기록합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-556">Log all operations (successful and failed), including compensating actions.</span></span> <span data-ttu-id="4baf9-557">상관 관계 ID를 사용하여 동일한 트랜잭션 내의 모든 작업을 추적할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-557">Use correlation IDs, so that you can track all operations within the same transaction.</span></span>

### <a name="a-call-to-a-remote-service-fails"></a><span data-ttu-id="4baf9-558">원격 서비스에 대한 호출이 실패합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-558">A call to a remote service fails.</span></span>
<span data-ttu-id="4baf9-559">**검색**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-559">**Detection**.</span></span> <span data-ttu-id="4baf9-560">HTTP 오류 코드입니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-560">HTTP error code.</span></span>

<span data-ttu-id="4baf9-561">**복구**</span><span class="sxs-lookup"><span data-stu-id="4baf9-561">**Recovery**</span></span>

1. <span data-ttu-id="4baf9-562">일시적 장애 시 다시 시도합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-562">Retry on transient failures.</span></span>
2. <span data-ttu-id="4baf9-563">*N*회의 시도 후에 호출이 실패하면 대체(fallback) 작업을 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-563">If the call fails after *N* attempts, take a fallback action.</span></span> <span data-ttu-id="4baf9-564">(응용 프로그램 특정입니다.)</span><span class="sxs-lookup"><span data-stu-id="4baf9-564">(Application specific.)</span></span>
3. <span data-ttu-id="4baf9-565">연속 장애를 방지하도록 [회로 차단기 패턴][circuit-breaker]을 구현합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-565">Implement the [Circuit Breaker pattern][circuit-breaker] to avoid cascading failures.</span></span>

<span data-ttu-id="4baf9-566">**진단**.</span><span class="sxs-lookup"><span data-stu-id="4baf9-566">**Diagnostics**.</span></span> <span data-ttu-id="4baf9-567">모든 원격 호출 실패를 기록합니다.</span><span class="sxs-lookup"><span data-stu-id="4baf9-567">Log all remote call failures.</span></span>

## <a name="next-steps"></a><span data-ttu-id="4baf9-568">다음 단계</span><span class="sxs-lookup"><span data-stu-id="4baf9-568">Next steps</span></span>
<span data-ttu-id="4baf9-569">FMA 프로세스에 대한 자세한 내용은 [클라우드 서비스 디자인에 의한 복원력][resilience-by-design-pdf](PDF 다운로드)을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="4baf9-569">For more information about the FMA process, see [Resilience by design for cloud services][resilience-by-design-pdf] (PDF download).</span></span>
