...
**자동화된 보안 침투 테스트를 수행합니다.** 응용 프로그램이 보안 상태를 유지하는 것은 다른 기능을 테스트하는 것만큼 중요합니다. 자동화된 침투 테스트를 빌드 및 배포 프로세스의 표준 파트로 만듭니다. 배포된 응용 프로그램에서 열린 포트, 엔드포인트 및 공격을 모니터링하여 기본 보안 테스트 및 취약성 검사를 예약합니다. 자동화된 테스트를 실행하더라도 정기적으로 심층 보안 검토가 필요합니다.
