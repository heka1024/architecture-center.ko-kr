...
대부분의 클라우드 호스팅 환경에서는 애플리케이션의 리소스 및 정적 페이지 일부를 스토리지 서비스에 배치하여 계산 인스턴스에 대한 필요성을 최소화할 수 있습니다(예를 들어 더 작은 인스턴스 또는 더 적은 수의 인스턴스 사용). 클라우드에 호스트된 저장소의 비용은 일반적으로 계산 인스턴스의 경우보다 훨씬 저렴합니다.
...
- 저장소 계정에 있는 콘텐츠도 있고, 호스트된 계산 인스턴스에 있는 콘텐츠도 있는 경우, 애플리케이션을 배포하고 업데이트하는 것이 더 어려워집니다. 보다 쉬운 관리를 위해 배포를 개별로 수행하고 애플리케이션 및 콘텐츠의 버전을 관리해야 할 수 있습니다&mdash;(특히 정적 콘텐츠에 스크립트 파일 또는 UI 구성 요소가 포함되어 있는 경우). 그러나 정적 리소스만 업데이트해야 할 경우에는, 애플리케이션 패키지를 다시 배포하지 않고도 해당 리소스를 저장소 계정에 업로드하기만 하면 됩니다.
...
각 페이지의 링크는 리소스의 URL을 지정하며, 클라이언트는 저장소 서비스에서 리소스에 직접 액세스합니다. 다음 그림에서는 저장소 서비스에서 직접 애플리케이션의 정적 부분을 전달하는 모습을 보여 줍니다.
