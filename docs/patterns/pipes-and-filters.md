...
애플리케이션은 처리되는 정보의 복잡도가 다양한 여러 작업을 수행해야 합니다. 간단하면서도 유연성 없는 애플리케이션 구현 방식은 이 작업을 모놀리식 모듈로 수행합니다. 그러나 이러한 방식은 애플리케이션 내의 다른 부분에서 중복된 처리가 필요한 경우에서 코드 리팩터링, 최적화 및 재사용의 기회를 줄일 수 있습니다.
