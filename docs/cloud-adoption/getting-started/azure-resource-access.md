...
Azure 구독은 리소스 그룹 및 해당 리소스를 함께 그룹화하는 논리적 구문에서 리소스 그룹과 비슷합니다. 그러나 Azure 구독은 Azure Resource Manager에서 사용한 컨트롤과도 연결됩니다. 무슨 의미인가요? 이와 Azure 구독 간의 관계에 대해 자세히 알아보기 위해 Resource Manager에 대해 더 자세히 살펴보겠습니다.
...
[Azure의 작동 방식](what-is-azure.md)에서 Azure에는 Azure의 모든 기능을 오케스트레이션하는 여러 서비스를 사용하는 "프런트 엔드"가 포함된다는 사실을 알아보았습니다. [Resource Manager](/azure/azure-resource-manager/)는 이러한 서비스 중 하나이며 이 서비스는 리소스를 관리하기 위해 클라이언트에서 사용하는 RESTful API를 호스트합니다.
...
*그림 5. Resource Manager RESTful API에 대한 Azure 클라이언트 연결.*

이러한 클라이언트가 RESTful API를 사용하여 Resource Manager에 연결하는 반면 Resource Manager에는 리소스를 직접 관리하는 기능이 포함되지 않습니다. 대신 Azure에서 대부분의 리소스 종류에는 고유한 [**리소스 공급자**](/azure/azure-resource-manager/resource-group-overview#terminology)가 포함됩니다.

*
*그림 6. Azure 리소스 공급자.*

클라이언트가 특정 리소스를 관리하도록 요청하면 Resource Manager는 요청을 완료하기 위해 해당 리소스 종류에 대한 리소스 공급자에 연결합니다. 예를 들어 클라이언트가 가상 머신 리소스를 관리하도록 요청하는 경우 Resource Manager는 **Microsoft.compute** 리소스 공급자에 연결합니다.

![](../_images/governance-1-15.png)
*그림 7. Resource Manager는 **Microsoft.Compute** 리소스 공급자에 연결하여 클라이언트 요청에 지정된 리소스를 관리합니다.*

해당 Resource Manager에서는 클라이언트가 가상 머신 리소스를 관리하기 위해 구독 및 리소스 그룹 모두에 대한 식별자를 지정해야 합니다.

이제 Resource Manager 작동 방식을 이해했으므로 Azure Resource Manager에서 사용하는 컨트롤과 Azure 구독을 연결하는 방법을 다시 살펴보겠습니다. Resource Manager가 리소스 관리 요청을 실행하기 전에 컨트롤 세트를 확인합니다.

첫 번째 컨트롤은 유효성이 검사된 사용자가 요청해야 하고 Resource Manager에게는 사용자 ID 기능을 제공하기 위해 [Azure AD(Azure Active Directory)](/azure/active-directory/)를 사용하는 신뢰할 수 있는 관계가 있습니다.
...
최종 컨트롤은 요청이 구독과 연결된 재무 약정 내에 있는지를 검사합니다. 예를 들어 요청이 가상 머신을 배포하는 것인 경우 Resource Manager는 구독에 충분한 결제 정보가 있는지 확인합니다.
...
이제 Azure에서 리소스 액세스를 관리하는 방법을 이해했으므로 거버넌스 모델을 디자인하는 방법을 알아보겠습니다.

> [!div class="nextstepaction"]
> [클라우드 거버넌스](../governance/overview.md)
